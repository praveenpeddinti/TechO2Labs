...
class ERestController extends Controller
{
    ...
    public function filterRestAccessRules( $c )
	{
                ...
                if( isset($_REQUEST['isAjax'])) 
		{
                    try{
                        Yii::app()->errorHandler->errorAction = '/' . $this->uniqueid . '/error';
                        if(!(isset($_REQUEST['X_REST_USERNAME']) and isset($_REQUEST['X_REST_PASSWORD']))) {
				throw new CHttpException(401, 'You are not authorized to preform this action.');
			}
                        $username = $_REQUEST['X_REST_USERNAME'];
		        $password = $_REQUEST['X_REST_PASSWORD'];
			...
			$c->run(); 
                        ...
                        $JSON = CJSON::encode(array(
                            ...
                            'data'=>array('errorCode'=>$ex->getCode())
                        ));
 ...
 /**
	...
	public function actionRestCreate($id=null, $var=null) 
	{
		...
		else
		{
			...
			if(method_exists($this, $var))
				...
				$this->triggerCustomRestCreate($id, $var);
		}
	}
	...
	*/
	...
	public function triggerCustomRestGet($id, $vars=array())
	{
        ...
        public function triggerCustomRestCreate($id, $var="")
	{
		$method = 'action' . ucfirst($id);
		if(method_exists($this, $method))
			$this->$method($var);
	...
	******************************************************************************************
	...
	public function validateAjaxUser($action)
	{
            ...
            Yii::log("validate ajax user",'error','application');
		return false;
	}
