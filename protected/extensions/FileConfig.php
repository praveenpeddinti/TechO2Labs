...
class FileConfig extends CApplicationComponent
{
	...
	public function setValue($key, $value, $forceWrite = FALSE)
	{
		...
		if ($cache !== FALSE) $cache->set(self::CACHE_KEY, $this->_config);
                ...
                error_log("###########Exception while file write/read=111111111111111111====".$ex->getMessage());
            }
	}
	...
	public function setConfig() {
        ...
        try {
            ...
            if ($handle) {
                ...
                if (fwrite($handle, "<?php return " . str_replace("]","",str_replace("[","",print_r($this->_config, true))) . "?>")) {
                    if ($this->_getCache() != FALSE)
                        $cache->set(self::CACHE_KEY, $this->_config);

                    return TRUE;
                } else
                    throw new CException('Error writing configuration file "' . $file . '"! ' . self::ERROR_MSG);
            } else
                throw new CException('Error opening configuration file "' . $file . '" for writing! ' . self::ERROR_MSG);
        } catch (Exception $ex) {
            error_log("###########Exception while file write/read=====" . $ex->getMessage());
        }
    }
	...
	public function getConfig()
	{
		...
		if (($cache === FALSE) || ($this->_config = $cache->get(self::CACHE_KEY) === FALSE))
		{
			if (is_readable($file))
			{
/*	commented this line, due to the exceptions are raised while reading
 *				$this->_config = unserialize(file_get_contents($file));
 * 
 */
                                $this->_config = file_get_contents($file);
