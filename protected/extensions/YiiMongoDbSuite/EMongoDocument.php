			...
			if ('_id' === $pkField) {
				foreach ($pk as $key => $value) {
					if ((strlen($value) === 24) && !$value instanceof MongoId) {
						...
						Yii::trace(get_class($this).".createPkCriteria() .. converting key value ($value) to MongoId",'ext.MongoDb.EMongoDocument');
						$pk[$key] = new MongoId($value);
					} elseif (is_numeric($value)) {
						// Assumption: need to bless as int, as string != int when looking up primary keys
						Yii::trace(get_class($this).".createPkCriteria() .. casting ($value) to int",'ext.MongoDb.EMongoDocument');
						$pk[$key] = (int)$value;
					}
				}
			}
