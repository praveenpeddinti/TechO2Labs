...
class Techo2EmployeeController extends Controller {
    ...
    //To Handle Error Actions
    public function actionError() {

        $this->render('/Employee/error');
    }

    //New Registration And Login Section
    public function actionEmployeeRegNLogin() {
        $data = array();
        $data['pageTitle'] = Yii::app()->params['pageTitles']['home'];
        $loginErrors = array();
        $regErrors = array();
        ...
        if (isset($_POST['EmployeeLoginModelForm'])) {
            ...
            if ("[]" != $loginErrors) {
                ...
                if (isset($loggedInAccArr) && count($loggedInAccArr) > 0) {
                    ...
                    Yii::app()->user->setFlash('loginFail', 'Invalid username or password.');
        ...
        if (isset($_POST['EmployeeRegModelForm'])) {
            ...
            if ("[]" != $regErrors) {
                ...
                if ($responseOnCreation > 0) {
                    Yii::app()->user->setFlash('registrationSuccess', 'Thank you for registering with us.');
                    $this->redirect(array('Techo2Employee/EmployeeRegNLogin'));
                } else {
                    Yii::app()->user->setFlash('registrationFail', 'Registration failed. Please try again after some time.');
                    $this->redirect(array('Techo2Employee/EmployeeRegNLogin'));
                }
            }
        }
    ...
    //Get States List By Passing Country Id
    ...
    public function actionDashboard() {
        ...
        if (0 == count($session)) {
            ...
            $data = array();
            $data['pageTitle'] = Yii::app()->params['pageTitles']['dashboard'];
    ...
    public function actionEditEmployeeProfile() {
        ...
        if (NULL == $_GET['employee_id'] || $_GET['employee_id'] <= 0) {
            ...
            $data = array();
            $data['pageTitle'] = Yii::app()->params['pageTitles']['editProfile'];
            ...
            //Email Address
            // $emp_unqi_emp_address = isset($employeeProfileDet['employee_email']) ? $employeeProfileDet['employee_email'] : NULL;
            // $editEmployeeProfileVal->techo2_Emp_Email = $emp_unqi_emp_address;
            //Contact Number
            // $emp_unqi_mobile_number = isset($employeeProfileDet['employee_phone']) ? $employeeProfileDet['employee_phone'] : NULL;
            // $editEmployeeProfileVal->techo2_Emp_Phone = $emp_unqi_mobile_number;
            //Get All Genders Start
            $genderRes = ServiceFactory::getAllActiveGendersList()->getGendersList();
            if (isset($genderRes) && count($genderRes) > 0) {
                $genderArr = $genderRes;
            ...
            if (isset($_POST['EditEmployeeProfileForm'])) {
                ...
                if ("[]" != $profileUpdateErrors) {
                    ...
                    if (1 == $res_on_employee || 1 == $res_on_address || 1 == $res_on_email || 1 == $res_on_phone) {
                        ...
                        Yii::app()->user->setFlash('noChange', 'Nochanges are made.');
                        $this->redirect(array('Techo2Employee/EditEmployeeProfile', 'employee_id' => $employee_id));
                    } else {
                        Yii::app()->user->setFlash('Oops', 'Ooops error occured. Please try again later.');
                        $this->redirect(array('Techo2Employee/EditEmployeeProfile', 'employee_id' => $employee_id));
                    }
                }
            }
     ...
     $res_on_email_comp = strcmp($employee_email, $hidden_email);
        ...
        if (0 != $res_on_email_comp) {
            ...
            if (isset($email_response) && count($email_response) > 0) {
                $response['invalidEmail'] = "Email address already exists. Please try with another one.";
            }
        }
      ...
      $res_on_phone_comp = strcmp($employee_phone, $hidden_phone);
        if (0 != $res_on_phone_comp) {
            ...
            if (isset($phone_response) && count($phone_response) > 0) {
                $response['invalidPhone'] = "Mobile number already exists. Please try with another one.";
            }
        }
